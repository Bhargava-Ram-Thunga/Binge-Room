...
| ---------------------------- | -------------------------------------------------- | -------------------------------------------------- | ----------------------------------- | ---------------------------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Manifest V3 Runtime**      | Background Service Worker                          | SUPPORTED                                          | SUPPORTED                           | PARTIAL (supports both background scripts and event pages) | SUPPORTED                                     | _Blocker_: Chromium MV3 terminates idle service workers after 30 seconds.<br>_Workaround_: Maintain the central WebSocket in the Background Service Worker with a 20-second port keep-alive heartbeat (`chrome.runtime.Port`) from active tabs (ADR-010 Option B). |
...
1. **Persistent WebSocket Gateway Separation & Lifecycle Management**:
   - The authoritative WebSocket gateway connection lives in the Background Service Worker (`apps/extension`).
   - To prevent Chromium MV3's 30-second idle termination, active watch-party content scripts establish a long-lived `chrome.runtime.Port` connection and maintain a 20-second heartbeat keep-alive exchange.
   - Content scripts communicate bidirectional synchronization events with the Background Service Worker over `chrome.runtime.Port`, preserving room connectivity across in-tab page navigations and centralizing room state.
