...
export default function CreateRoom({ onBack }: Props) {
  ...
  const handleCreate = async () => {
    ...
    const res = await roomService.createRoom(name.trim(), videoUrl || undefined);
    setCreating(false);
    if (!res.success) setError(res.error ?? 'Failed to create room. Is the server running?');
  };
  ...
  return (
    <div className="animate-fadeSlideUp" style={{ paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {/* Title */}
      <div>
        <h2 style={{ fontSize:15, fontWeight:700, color:'#f1f5f9', marginBottom:2 }}>Create a Room</h2>
        <p style={{ fontSize:12, color:'#475569' }}>Share the 6-digit code with friends.</p>
      </div>
      ...
      {/* Video card */}
      {videoId ? (
        <div style={{ borderRadius:10, overflow:'hidden', border:'1px solid rgba(99,102,241,.2)', background:'rgba(15,23,42,.8)' }}>
          <img
        ...
        <div style={{ padding:'10px 12px', borderRadius:10, background:'rgba(245,158,11,.07)', border:'1px solid rgba(245,158,11,.2)', display:'flex', gap:8, alignItems:'flex-start' }}>
          <span style={{ fontSize:14, flexShrink:0 }}>⚠️</span>
          <p style={{ fontSize:11, color:'#fbbf24', lineHeight:1.5 }}>
            Open a YouTube video first — everyone in the room will be sent to the same video.
          </p>
        </div>
      )}
