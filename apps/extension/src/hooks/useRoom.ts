...
export function useRoom() {
  ...
  useEffect(() => {
    ...
    const handler = (message: {
      ...
      switch (message.type) {
        case "ROOM_UPDATE":
          if ("room" in message.payload)
            store.setRoom((message.payload.room as Room) ?? null);
          if ("user" in message.payload)
            store.setCurrentUser((message.payload.user as User) ?? null);
          break;
        case "CONNECTION_STATUS":
          store.applyConnectionStatus(
            message.payload as unknown as ConnectionStatus,
          );
